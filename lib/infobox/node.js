define(["chroma-js", "moment", "numeral", "tablesort", "tablesort.numeric"],
  function (chroma, moment, numeral, Tablesort) {
  ...
  function showWifiUsage(d) {
    if (!("wifistats" in d.nodeinfo))
      return undefined

    return function (el) {
      el.appendChild(showBar("wifistats", d.nodeinfo.wifistats.total.utilization))
    }
  }
  ...
  function showBar(className, v) {
    var span = document.createElement("span")
    var linkScale = chroma.scale(chroma.interpolate.bezier(["#04C714", "#FF5500", "#F02311"])).domain([15, 95])

    span.classList.add("bar")
    span.classList.add(className)
    span.setAttribute("style", "background-color: rgba(" + linkScale(Math.round(v * 100)).rgb() + ", 0.8)" )

    var bar = document.createElement("span")
    bar.style.width = (v * 100) + "%"
    bar.style.backgroundColor = "rgba(" + linkScale(Math.round(v * 100)).rgb() + ", 1.0)"
  ...
  return function(config, el, router, d) {
    ...
    attributeEntry(attributes, "WLAN-Kanalbelegung", showWifiUsage(d))
